...
Read this from `init-harness` Step 8b (first-time install and upgrade mode
...
| `@next/next` (`@next/eslint-plugin-next`) | only when the detected framework is Next.js | part of the server/client import boundary goes unchecked until a manual code review pass catches it |
...
Determine this the same way as the rest of the check, by reading what the
project actually has, not by assuming: an ESLint config file (`.eslintrc*`,
`eslint.config.*`) or an `eslint` devDependency means ESLint; a `biome.json`/
`biome.jsonc` file, or no linter config at all, means it isn't.

...
4. Read only the slice of `contextFiles` this run actually needs, not the
   ...
   - `openspec/changes/<change>/ui-plan.md` — if this run's group(s) touch
     the interface (§3 determines which): read it before writing any
     component's code, not after. Take each screen's existing components by
     the names ui-plan.md gives them from `docs/design-system.md`; write a
     new component only where its row explicitly names one as new — never
     invent a new component ui-plan.md's table doesn't call for.
     ...
     that node's variables from the connected tool (color, spacing, font) —
     not only the system tokens in `docs/design-system.md`. No node, or no
     connected tool → as before: system tokens plus the row's text
     description.
